...
class DPOTrainer(Trainer):
    ...
    def __init__(
        ...
        #print("................................")
        #print(args.precompute_ref_log_probs)
        if ref_model:
    ...
    def tokenize_row(self, feature, model: Optional[Union[PreTrainedModel, nn.Module]] = None) -> Dict:
        ...
        for k in batch:
            batch[k] = [x for x in batch[k] if x is not None or x == 0]
            if k.startswith("chosen"):
                ...
                #print(batch[k])
                concatenated_batch[concatenated_key] = pad_to_length(torch.tensor(batch[k],dtype=torch.bfloat16), max_length, pad_value=pad_value)
        for k in batch:
            batch[k] = [x for x in batch[k] if x is not None or x == 0]
            if k.startswith("rejected"):
                ...
                concatenated_batch[concatenated_key] = torch.cat(
                    (
                        ...
                        pad_to_length(torch.tensor(batch[k],dtype=torch.bfloat16), max_length, pad_value=pad_value),
                    ),
                    dim=0,
                )
        print(".....................................................")
    ...
    def concatenated_image_inputs(
        ...
        for k in batch:
            if k.startswith("chosen") and isinstance(batch[k], torch.Tensor):
                ...
                chosen_batch[concatenated_key] = concatenated_batch[concatenated_key].repeat_interleave(2,0)
        ...
        for it in batch["images"]:
            it = it.to(device=device)
        '''
        concatenated_batch["images"] = [val for val in batch["images"] for i in range(2)]

        #concatenated_batch["rejected_images"] = []

        for x,y in zip(batch["images"],batch["rejected_images"]):
            concatenated_batch["images"].append(x)
            concatenated_batch["images"].append(y)
        #print("==================================================================")
        #print(concatenated_batch)
        #print(type(concatenated_batch["concatenated_input_ids"]))
    ...
    def concatenated_inputs(
        ...
        for k in batch:
            if k.startswith("chosen") and isinstance(batch[k], torch.Tensor):
                ...
                concatenated_batch[concatenated_key] = pad_to_length(batch[k], max_length, pad_value=pad_value)
        #print("==================================================")
        #print(concatenated_batch["concatenated_input_ids"])
        for k in batch:
            if k.startswith("rejected") and isinstance(batch[k], torch.Tensor):
                if "labels" in k or is_encoder_decoder:
                    pad_value = label_pad_token_id
        ...
        for x,y in zip(batch["images"],batch["rejected_images"]):
            concatenated_batch["rejected_images"].append(x)
            concatenated_batch["rejected_images"].append(y)
        '''
        #print("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
        #print(concatenated_batch)
        return concatenated_batch
    ...
    def concatenated_image_forward(
        ...
        image_chosen_logps = all_logps[len_chosen*2:len_chosen*3]
        image_rejected_logps = all_logps[len_chosen*3:]

        chosen_logits = all_logits[:len_chosen]
        rejected_logits = all_logits[len_chosen:len_chosen*2]
        image_chosen_logits = all_logits[len_chosen*2:len_chosen*3]
        image_rejected_logits = all_logits[len_chosen*3:]

        if self.aux_loss_enabled:
            return (chosen_logps, rejected_logps, chosen_logits, rejected_logits, nll_loss,image_chosen_logps, image_rejected_logps, image_chosen_logits, image_rejected_logits, outputs.aux_loss)

        return (chosen_logps, rejected_logps, chosen_logits, rejected_logits, nll_loss,image_chosen_logps, image_rejected_logps, image_chosen_logits, image_rejected_logits)
    ...
    def concatenated_forward(
        ...
        len_chosen = batch["chosen_labels"].shape[0]
        '''
        batch_2 = self.concatenated_image_inputs(
            batch,
            is_encoder_decoder=self.is_encoder_decoder,
            is_vision_model=self.is_vision_model,
            label_pad_token_id=self.label_pad_token_id,
    ...
    def get_image_batch_loss_metrics(
        self,
        model,
        batch: Dict[str, Union[List, torch.LongTensor]],
        ...
        forward_output = self.concatenated_image_forward(model, batch)
        (
            policy_chosen_logps,
            ...
            policy_nll_loss,
            image_policy_chosen_logps,
            image_policy_rejected_logps,
            image_policy_chosen_logits,
            image_policy_rejected_logits,
        ) = forward_output[:9]
        if self.aux_loss_enabled:
                ...
                if self.ref_model is None:
                    with self.null_ref_context():
                        (
                            ...
                            _,
                            _,
                            image_reference_chosen_logps,
                            image_reference_rejected_logps,
                            _,
                            _,
                        ) = self.concatenated_image_forward(self.model, batch)
                else:
                    (
                        reference_chosen_logps,
                        reference_rejected_logps,
                        ...
                        _,
                        _,
                    ) = self.concatenated_image_forward(self.ref_model, batch)
        ...
        image_losses, image_chosen_rewards, image_rejected_rewards = self.dpo_loss(
            image_policy_chosen_logps,
            image_policy_rejected_logps,
            image_reference_chosen_logps,
            image_reference_rejected_logps,
    ...
    def get_batch_loss_metrics(
        ...
        ##add losses
        losses = losses + losses_image * 0.5
    ...
    def compute_loss(
        ...
        with compute_loss_context_manager():
            loss, metrics = self.get_image_batch_loss_metrics(model, inputs, train_eval="train")
    ...
    def prediction_step(
        ...
        with torch.no_grad(), prediction_context_manager():
            loss, metrics = self.get_image_batch_loss_metrics(model, inputs, train_eval="eval")
