...
class DpoDataset(BaseDataset):
    ...
    def __init__(self, datasets, tokenizer, multimodal_cfg):
        super(DpoDataset, self).__init__(datasets, tokenizer, multimodal_cfg)
        # v0 version format conversation
        conversation_lib.default_conversation = conversation_lib.conv_templates["mpt"]
        logging.warning("Formatting inputs into conversation type: mpt-fixed")
        logging.warning("Loading data...")
    ...
    def token_processor(self, sources):
        conv = conversation_lib.default_conversation.copy()
        conv_reject = conversation_lib.default_conversation.copy()
        #print(conv)
        roles = {"human": conv.roles[0], "gpt": conv.roles[1],"refer":conv.roles[1]}
        #print(sources[0])
        # Apply prompt templates
        conversations = []
        conversations_reject = []
        for i, source in enumerate(sources):
            if roles[source[0]["from"]] != conv.roles[0]:
                # Skip the first one if it is not from human
                source = source[1:]
            ...
            for j, sentence in enumerate(source):
                ...
                if j != 2:
                   conv.append_message(role, sentence["value"])
                if j != 1:
                    conv_reject.append_message(role, sentence["value"])
            conversations.append(conv.get_prompt())
            conversations_reject.append(conv_reject.get_prompt())

        
        #print(conversations[0])
        #print(conversations_reject[0])
        ...
        input_ids_reject = self.tokenizer(
            conversations_reject,
            return_tensors="pt",
            padding="longest",
            max_length=self.tokenizer.model_max_length,
        ...
        # Mask reject
        rejecteds = input_ids_reject.clone()
        assert conv_reject.sep_style == conversation_lib.SeparatorStyle.MPT

        sep = conv_reject.sep + conv_reject.roles[1]
        for conversation, target in zip(conversations_reject, rejecteds):
            total_len = int(target.ne(self.tokenizer.pad_token_id).sum())

            rounds = conversation.split(conv_reject.sep)
            re_rounds = [conv_reject.sep.join(rounds[:3])] # system + user + gpt
            for conv_idx in range(3, len(rounds), 2):
                re_rounds.append(conv_reject.sep.join(rounds[conv_idx:conv_idx+2]))    # user + gpt
            cur_len = 0
            target[:cur_len] = IGNORE_INDEX
            for i, rou in enumerate(re_rounds):
                if rou == "":
                ...
                if len(parts) != 2:
                    break
                parts[0] += sep
                round_len = len(self.tokenizer(rou).input_ids) + len(self.tokenizer(conv_reject.sep).input_ids)
        ...
        return dict(
            ...
            rejected_input_ids=input_ids_reject,
            chosen_labels=targets,
            rejected_labels=rejecteds
        )
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if isinstance(data, dict):
            ...
            if 'rejected_image' in data:
                rejected_image_file = data['rejected_image']

                try:
                    rejected_image = Image.open(rejected_image_file).convert('RGB')
                except:
                    print(f'cannot identify image file {rejected_image_file}.')
                    return self.__getitem__(0)
                
                try:
                    rejected_image, rejected_image_1 = self.image_processor(rejected_image)
                except:
                    print(f'image {rejected_image_file} are broken or grayscale! we thus select 0-th sample instead!')
                    return self.__getitem__(0)
        ...
        data_dict = dict(chosen_input_ids=data_dict["chosen_input_ids"][0], 
                         ...
                         rejected_input_ids=data_dict["rejected_input_ids"][0],
                         rejected_labels=data_dict["rejected_labels"][0])
        ...
        if isinstance(data, dict) and 'rejected_image' in data:
            data_dict['rejected_image'] = [rejected_image]
            data_dict['rejected_image_high'] = [rejected_image_1]
